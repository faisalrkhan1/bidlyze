...
import { TEXT_LIMITS, TENDER_FILE_ROLE_LABELS } from "@/lib/constants";
  ...
  "requirements": [
    {
      ...
      "sourceRef": "Page number, section number, or clause reference where this requirement appears (e.g. 'Page 12', 'Section 3.2', 'Clause 4.1.3')"
    }
  ],
  "complianceAnalysis": {
    ...
    "estimatedCompetitors": [
      {
        ...
        "speculative": true,
        "inferenceBasis": "Brief explanation of why this competitor was inferred (e.g. 'Active in similar GCC ITS tenders')"
      }
    ],
...
- For competitorIntelligence, provide AI-INFERRED, SPECULATIVE likely competitor archetypes based on tender sector, technology requirements, geographic location, project size, and known regional players. These are educated guesses, NOT verified market intelligence. The user MUST verify any competitor before relying on this data. Each competitor entry must include a 'speculative: true' flag and an 'inferenceBasis' field briefly explaining what you based the guess on (e.g. 'Inferred from regional ITS sector players in UAE'). Provide 3-5 inferred competitor archetypes.
...
- For requirements, always include "sourceRef" with the page number, section, or clause reference where the requirement appears in the document. If a specific page cannot be determined, use the section heading or "General" as the reference.
...
/**
 * Apportion supporting-doc text within MAX_TEXT_CHARS_SUPPORTING_TOTAL.
 * Each supporting doc gets its proportional share by original length,
 * but no doc gets more than total / N (fairness floor) so one giant BOQ
 * cannot crowd out the others.
 */
function budgetSupportingTexts(supportingDocs) {
  if (!supportingDocs.length) return [];
  const totalBudget = TEXT_LIMITS.MAX_TEXT_CHARS_SUPPORTING_TOTAL;
  const fairShareCap = Math.floor(totalBudget / supportingDocs.length);
  const totalLen = supportingDocs.reduce((s, d) => s + d.text.length, 0) || 1;

  // First pass: proportional share, capped by fair-share.
  const allocations = supportingDocs.map((d) => {
    const proportional = Math.floor((d.text.length / totalLen) * totalBudget);
    return Math.min(proportional, fairShareCap, d.text.length);
  });

  // Redistribute unused budget to docs that wanted more than their share.
  let leftover = totalBudget - allocations.reduce((s, a) => s + a, 0);
  let safety = supportingDocs.length;
  while (leftover > 0 && safety-- > 0) {
    let distributed = 0;
    for (let i = 0; i < supportingDocs.length; i++) {
      const remaining = supportingDocs[i].text.length - allocations[i];
      if (remaining > 0 && leftover > 0) {
        const grant = Math.min(remaining, Math.ceil(leftover / supportingDocs.length));
        allocations[i] += grant;
        leftover -= grant;
        distributed += grant;
      }
  ...
  return supportingDocs.map((d, i) => ({
    ...d,
    text: d.text.substring(0, allocations[i]),
  }));
}

/**
 * Tender Package analysis: many files, one unified output.
 *
 * @param {object[]} files            Array of { filename, role, text }
 * @param {string}   rfxType          rfp | rfq | rfi | other (drives system prompt)
 * @returns {Promise<{success, data?, error?}>}
 */
export async function analyzeTenderPackage(files, rfxType = "rfp") {
  try {
    if (!files?.length) {
      return { success: false, error: "No files provided to analyze." };
    }
    const primary = files.find((f) => f.role === "primary");
    if (!primary) {
      return { success: false, error: "Exactly one file must be designated as the primary RFP." };
    }
    const supporting = files.filter((f) => f.role !== "primary");

    // Apply token budgets
    const primaryText = primary.text.substring(0, TEXT_LIMITS.MAX_TEXT_CHARS_PRIMARY);
    const budgetedSupporting = budgetSupportingTexts(supporting);

    // Build the structured multi-doc block.
    const blocks = [];
    blocks.push(`=== PRIMARY DOCUMENT: ${primary.filename} ===\n${primaryText}`);
    for (const s of budgetedSupporting) {
      const label = TENDER_FILE_ROLE_LABELS[s.role] || "Other";
      blocks.push(`=== SUPPORTING DOCUMENT [${label}]: ${s.filename} ===\n${s.text}`);
    }

    const intro = `You are analyzing a TENDER PACKAGE with ${files.length} document${files.length === 1 ? "" : "s"}. The primary document is the main RFP/RFQ/RFI. Supporting documents (BOQ, annexes, T&Cs, drawings) provide context for specific requirements.\n\nWhen extracting requirements, indicate which document they come from in the sourceRef field (e.g. "Primary RFP - Page 12", "BOQ - Sheet 'Pricing' Row 5", "Annex A - Section 3.2").`;
    const multiDocBlock = `${intro}\n\n${blocks.join("\n\n")}`;
    ...
    console.error("Tender package analysis error:", error);
    return {
      success: false,
      error: "Failed to analyze tender package. Please try again.",
    };
  }
}
...
export async function analyzeTenderFromPDF(base64PDF, rfxType = "rfp") {
  try {
    ...
    return analyzeTender(text.substring(0, TEXT_LIMITS.MAX_TEXT_CHARS_SINGLE), rfxType);
  } catch (error) {
