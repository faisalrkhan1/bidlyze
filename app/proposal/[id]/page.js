...
export default function ProposalPage({ params }) {
  ...
  const [docxState, setDocxState] = useState({ status: "idle", error: "" });
  ...
  async function handleDownloadDocx() {
    if (docxState.status === "generating") return;
    if (Object.keys(sections).length === 0) return;

    setDocxState({ status: "generating", error: "" });
    try {
      const { exportProposalToDocx } = await import("@/app/utils/exportDocx");
      const summary = record?.analysis_data?.summary || {};
      await exportProposalToDocx({
        tenderTitle: summary.projectName || "Untitled Tender",
        tenderReference: summary.tenderReference || "",
        clientName: summary.issuingAuthority || "",
        companyName:
          user?.user_metadata?.company_name ||
          user?.user_metadata?.company ||
          "",
        sections: SECTIONS.filter((s) => sections[s.key]).map((s) => ({
          key: s.key,
          label: s.label,
          content: sections[s.key],
        })),
      });
      setDocxState({ status: "success", error: "" });
      setTimeout(() => {
        setDocxState((prev) =>
          prev.status === "success" ? { status: "idle", error: "" } : prev
        );
      }, 3000);
    } catch (err) {
      ...
      setDocxState({
        status: "error",
        error:
          "Could not generate Word document. Please try again or copy to clipboard.",
      });
      setTimeout(() => {
        setDocxState((prev) =>
          prev.status === "error" ? { status: "idle", error: "" } : prev
        );
      }, 5000);
    }
  }
    ...
    <AppShell user={user} onLogout={logout} breadcrumbs={[{label: "Dashboard", href: "/dashboard"}, {label: "Analysis", href: "/analysis/" + id}, {label: "Proposal Writer"}]}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              {generatedCount > 0 && (
                <button
                  ...
                  disabled={docxState.status === "generating"}
                  className="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors duration-300 flex items-center gap-1.5 disabled:opacity-60 disabled:cursor-not-allowed"
                  style={{ border: "1px solid #D4764E", background: "transparent", color: "#D4764E" }}
                  onMouseEnter={(e) => {
                    if (docxState.status !== "generating") {
                      e.currentTarget.style.background = "rgba(212, 118, 78, 0.08)";
                    }
                  }}
                  onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
                  title={
                    docxState.status === "error"
                      ? docxState.error
                      : "Download proposal as Microsoft Word document"
                  }
                >
                  {docxState.status === "generating" ? (
                    <>
                      <div className="w-3.5 h-3.5 border-2 border-current border-t-transparent rounded-full animate-spin" />
                      Generating...
                    </>
                  ) : docxState.status === "success" ? (
                    <>
          ...
          {docxState.status === "error" && (
            <div
              className="shrink-0 px-6 py-2 text-xs flex items-center gap-2"
              style={{
